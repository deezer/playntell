...
from updown.data.custom_readers import (CocoCaptionsPointerReader,
                                        ImageFeaturesReader)
# from updown.data.readers import CocoCaptionsReader, ConstraintBoxesReader, ImageFeaturesReader
from updown.types import (EvaluationBatch, EvaluationBatchWithConstraints,
                          EvaluationInstance,
                          EvaluationInstanceWithConstraints, TrainingBatch,
                          TrainingInstance)
...
class TrainingDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> TrainingInstance:
        ...
        caption_tokens: List[int] = [
            self._vocabulary.get_token_index(c) for c in caption
        ]
        ...
        detection_labels = [
            self._vocabulary.get_token_index(label) for label in detection_labels
        ]
    ...
    def collate_fn(self, batch_list: List[TrainingInstance]) -> TrainingBatch:
        ...
        image_id = torch.tensor(
            [instance["image_id"] for instance in batch_list]
        ).long()
        num_detections = torch.tensor(
            [instance["num_detections"] for instance in batch_list]
        ).long()
        ...
        (
            ...
            detection_labels,
            detection_classes,
        ) = _custom_collate_image_features(
            detection_features_list=[
                instance["detection_features"] for instance in batch_list
            ],
            detection_labels_list=[
                instance["detection_labels"] for instance in batch_list
            ],
            detection_classes_list=[
                instance["detection_classes"] for instance in batch_list
            ],
        )
...
class EvaluationDataset(Dataset):
    ...
    def __init__(
        self, vocabulary: Vocabulary, image_features_h5path: str, in_memory: bool = True
    ) -> None:
        # `in_memory` is left unused
        self._vocabulary = vocabulary
        self._image_features_reader = ImageFeaturesReader(image_features_h5path)
...
def _custom_collate_image_features(
    ...
    detection_classes_list: List[np.ndarray],
    ...
    detection_features = np.zeros(
        (len(detection_features_list), max(num_boxes), image_feature_size),
        dtype=np.float32,
    )

    detection_labels = np.zeros(
        (len(detection_features_list), max(num_boxes)), dtype=np.int64
    )
    ...
    for i, (instance_features, instance_labels, instance_classes, dim) in enumerate(
        zip(
            detection_features_list,
            detection_labels_list,
            detection_classes_list,
