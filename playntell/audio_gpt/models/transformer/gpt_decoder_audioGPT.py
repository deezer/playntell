...
from audio_gpt.models.containers import Module, ModuleList
...
class Attention(Module):
    ...
    def forward(self, x, layer_past=None, mask_self_attention=None):
        ...
        if self.can_be_stateful and self._is_stateful:
            self.running_keys = torch.cat(
                [self.running_keys, key.transpose(-2, -1)], -2
            )
...
class Enc_Dec_Attention(Module):
    ...
    def _attn(self, q, k, v, enc_dec_attention):
        ...
        # w = w * b - 1e10 * (1 - b)
        w = nn.Softmax(dim=-1)(w)
        enc_dec_attn_mask = w.detach().clone()
        w = self.attn_dropout(w)
        return torch.matmul(w, v), enc_dec_attn_mask
    ...
    def forward(self, x, layer_past=None, encoder_output=None, mask_encoder=None):
        ...
        a, enc_dec_attn_mask = self._attn(
            query, encoder_key, encoder_value, mask_encoder
        )
        a = self.merge_heads(a)
        a = self.c_proj(a)
        return a, enc_dec_attn_mask
...
class Block(Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        nx = config.n_embd
        self.srau = config.srau
        self.n_modes = config.n_modes

        self.ln_1 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = Attention(nx, n_ctx, config, scale, can_be_stateful=True)
        self.enc_dec_attn = ModuleList(
            [Enc_Dec_Attention(nx, n_ctx, config, scale) for _ in range(self.n_modes)]
        )
        ...
        self.fc_alpha = nn.Linear(nx + nx, nx)
    ...
    def forward(
        ...
        encoder_output=None,
        mask_encoder=None,
        mask_self_attention=None,
        tau=0,
    ):
        threshold = tau

        encoder_output = torch.split(
            encoder_output, encoder_output.shape[-1] // self.n_modes, dim=-1
        )
        mask_encoder = torch.split(
            mask_encoder, mask_encoder.shape[-1] // self.n_modes, dim=-1
        )

        # masked self-attentoion + add & norm
        self_attention, present = self.attn(
            self.ln_1(x), layer_past=layer_past, mask_self_attention=mask_self_attention
        )
        a = x + self_attention
        a = self.resid_pdrop(a)

        # enc-dec attention + mode fusion
        # every mode does cross-attention
        enc_atts, enc_dec_attn_masks = ([] for _ in range(2))

        for i in range(self.n_modes):

            enc_att, enc_dec_attn_mask = self.enc_dec_attn[i](
                x=self.ln_1(a),
                encoder_output=self.ln_1(encoder_output[i]),
                mask_encoder=mask_encoder[i],
            )

            enc_atts.append(enc_att)
            enc_dec_attn_masks.append(enc_dec_attn_mask)

        enc_att = torch.sum(torch.stack(enc_atts, dim=0), dim=0)
        enc_dec_attn_mask = torch.stack(enc_dec_attn_masks, dim=0)

        # linguistic and musical fusion through srau
        if self.srau:
            alpha = torch.sigmoid(self.fc_alpha(torch.cat([a, enc_att], -1)))

            linguistics_alpha_mask = torch.where(
                alpha > threshold, torch.ones_like(alpha), torch.zeros_like(alpha)
            )

            visual_alpha_mask = torch.where(
                alpha < 1 - threshold, torch.ones_like(alpha), torch.zeros_like(alpha)
            )

            enc_att = (
                alpha * linguistics_alpha_mask * a
                + (1 - alpha) * visual_alpha_mask * enc_att
            )
        ...
        return encoder_result, present, enc_dec_attn_mask
...
class GPT2Model(Module):
    ...
    def forward(
        ...
        for block, layer_past in zip(self.h, past):
            hidden_states, present, enc_dec_attn_mask = block(
                hidden_states,
                layer_past,
        ...
        return hidden_states.view(*output_shape), presents, enc_dec_attn_masks
...
class GPT2LMHeadModel(Module):
    def __init__(self, config, padding_idx=47932, tau=0):
        ...
        self.register_state(
            "running_mask_self_attention", torch.zeros((1, 1, 0)).bool()
        )
        self.tau = tau
    ...
    def forward(
        ...
        mask_self_attention = torch.triu(
            torch.ones((seq_len, seq_len), dtype=torch.uint8, device=input_ids.device),
            diagonal=1,
        )
        ...
        mask_self_attention = (
            mask_self_attention
            + (input_ids == self.padding_idx).unsqueeze(1).unsqueeze(1).bool()
        )
        ...
        hidden_states, presents, enc_dec_attn_masks = self.transformer(
            input_ids,
            position_ids,
            token_type_ids,
            past,
        ...
        return lm_logits, presents, enc_dec_attn_masks
